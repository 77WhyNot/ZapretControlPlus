...
| **Проверка VPN** | Кнопка показывает, каким адресом вас видят снаружи и из какой страны — через сам туннель, а не мимо него. Если адрес совпал с вашим настоящим, программа прямо скажет, что туннель не работает. |
| **Проверка доступности** | ChatGPT, Claude, Gemini, Discord, YouTube и Google — за один клик видно, что открывается, а что нет. |
...
Подойдёт подписка любого сервиса, лишь бы он выдавал ссылку или ключ
`vless://`. Если своей нет, на этой же вкладке есть кнопка с рекомендацией —
сервис сторонний, к программе отношения не имеет.

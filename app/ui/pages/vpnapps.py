...
class VpnAppsPage(Page):
    ...
    def _build_mode(self) -> None:
        ...
        from app.ui.pages.base import Banner

        self.banner_proxy = Banner(
            self.context, "info",
            "Сейчас VPN подключается как прокси — выбор программ не действует, "
            "через VPN идёт всё, что использует системный прокси. Чтобы "
    ...
    def _update_summary(self) -> None:
        ...
        if running:
            self.restart_hint.setText(
                "VPN сейчас работает — изменения применятся через пару секунд, "
                "туннель перезапустится сам."
            )
    ...
    def on_activate(self) -> None:
        ...
        self.banner_proxy.setVisible(
            vpn_actions.transport() == vpn_config.TRANSPORT_PROXY
        )
        self._sync_mode()
        self._reload_cards()
